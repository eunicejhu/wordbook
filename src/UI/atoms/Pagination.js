...
class Pagination extends Component {
  ...
  getVisiblePages = () => {
    ...
    if (current <= MAX_VISIBLE_PAGES / 2) {
      visiblePages = initArrayByLength(MAX_VISIBLE_PAGES).map((item, index) => index)
    } else if (current > (pages - MAX_VISIBLE_PAGES / 2 -1)) {
      visiblePages = initArrayByLength(MAX_VISIBLE_PAGES).map((item, index) => pages -1  - index).reverse()
    } else {
      let precedentIncludeCurrentArray, latterExcludeCurrentArray
      precedentIncludeCurrentArray = initArrayByLength(MAX_VISIBLE_PAGES / 2 + 1).map((item, index) => current - index).reverse()
      latterExcludeCurrentArray = initArrayByLength(MAX_VISIBLE_PAGES / 2 - 1).map((item, index) => current + index + 1)

      visiblePages = precedentIncludeCurrentArray.concat(latterExcludeCurrentArray)
    }
    return visiblePages
  }
  ...
  render() {
    ...
    renderVisiblePages = this.getVisiblePages().map((page, index) => (<a
      key={index}
      onClick={(current != page) ? (() => { this.onClickPageHandler(page); }) : () => {}}
    ...
    return (<div className="pagination-wrapper" >
      {hasPrevious && <div className="arrow" onClick={this.onClickPrevHandler}> <ArrowLeft /></div>}
      <div className="pages-wrapper" >{renderVisiblePages}</div>
      {hasNext && <div className="arrow" onClick={this.onClickNextHandler }><ArrowRight /></div>}
    </div>)
  }
}
